...
uv run python scripts/compare.py              # agreement matrix + cross-sorter array/yield metrics -> outputs/comparison.html
uv run python scripts/show_channels.py        # collection-sites probe map + per-channel traces -> outputs/probe_map.png, channels.png
uv run python scripts/show_channels.py --sorter mountainsort5 --channels 3,7,11   # highlight active electrodes; only these channels
...
`scripts/report.py` builds a single self-contained `outputs/report.html` (Plotly JS **inlined**, so it opens offline) via `build_report(data_dir, analyzer_dir, out_path)`. Each loader stage runs in its own try/except (a failure becomes a red/SKIP row in the status banner, never a crashed report), and sorted-unit data — sorting, waveform templates, quality metrics — is read **only** from the saved `SortingAnalyzer` (its single source of truth; the loose `outputs/<sorter>/sorting/` folder and `quality_metrics.csv` are from other runs and ignored). Sections: status banner, LFP traces + Welch spectrum, `.nev` online units, sorted units (raster/rates/templates), **array/yield summary** (the six metrics, from `sort_summary`), **probe geometry & channels** (`_render_probe`: collection-sites scatter coloured by per-channel noise, active electrodes ringed), quality metrics (sortable table + SNR scatter), event-marker timeline, footer. The interactive front door is now `SpikeInterface_Menu.py` (see below); `scripts/make_report.py` is a thin shim that forwards to it.

`scripts/sort_summary.py` is the **single source of truth for the array/yield headline metrics** the lab wants out of every sort: **V_pp** (peak-to-peak template amplitude per unit), **SNR**, **noise floor** (per-channel), **yield** (% of electrodes that are the peak channel of ≥1 unit), **units/ch** and **units/active-ch**. `compute_summary(analyzer)` builds the record; `write_summary` persists `summary.json` (full) + `summary.csv` (one headline row) next to the sort; `load_summary`/`headline_row`/`format_card`/`csv_row` are **pure** (no SpikeInterface / NumPy at import) so the Textual menu can render a saved summary. Amplitudes are physical **µV**: the `SortingAnalyzer` is created with `return_in_uV=True` (the SI default), so `templates`/`noise_levels` are already scaled — `compute_summary` gates on `analyzer.return_in_uV` and does **not** re-apply the channel gain (doing so once double-scaled V_pp/noise ~4× too small; the noise floor lands at ~4 µV for *every* sorter, since it is a recording property, which is the invariant to check after touching this). The summary is surfaced on all four places: the `run_sorting` terminal card + `summary` progress event, `report.html`, the menu INSPECTING panel, and the cross-sorter table in `comparison.html` (`compare._metrics_section`, every saved sort side-by-side).

**Sort failures are made legible** (was: a blank "sort exited (1) without finishing"). The quality-metrics phase in `run_sorting.py` is **non-fatal** — the sort is saved first, so a metrics crash degrades to a success-with-`note` (rc 0, real error surfaced, stale derived files cleared) instead of discarding the units; a **top-level guard** in `__main__` emits a real JSON `error` event (with the exception type+message) for any uncaught exception; and `SortProgressScreen` captures the subprocess stderr to `outputs/<sorter>/sort.log` (was `DEVNULL`) and shows its tail on a hard crash.

`scripts/show_channels.py` shows **different channels and collection sites** headlessly: `probe_map.png` (each electrode contact at its physical (x, y) µm position, labelled by channel, coloured by noise floor, active electrodes ringed) + `channels.png` (band-passed traces of all/`--channels`-selected channels, stacked in depth order) + a printed site→channel table. For interactive channel/probe browsing the menu's `traces` action opens **ephyviewer** and `gui` opens **spikeinterface-gui**.
